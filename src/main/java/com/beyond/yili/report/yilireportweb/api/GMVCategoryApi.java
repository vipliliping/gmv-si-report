...
/**
 * @author vipliliping
 * @create 2019/4/23 16:17
 * @desc
 **/
@Controller
@Api(
        value = "GMV品类明细",
        description = "GMV品类明细Api"
)
public class GMVCategoryApi {
    private static final Logger log = LoggerFactory.getLogger(GMVCategoryApi.class);
    private String[] cols = new String[]{"品类", "品牌", "SKU", "平台", "店铺", "成交商品件数(单位:件)", "成交金额(单位:万元)", "平均件单价(单位:元/件)"};
    ...
    @RequestMapping(
            value = {"/gmv/cate/detail"},
            method = {RequestMethod.POST},
            produces = {"application/json;charset=UTF-8"}
    )
    @ResponseBody
    public Object gmvCate(@ApiParam(name = "params",hidden = true) Map<String, Object> params, TreeGridRequestEnum typeEnum, String level, String treeValue, String date, String timeWD, String cates, String brands, String skus, String ranks, String plats, String shops) throws Exception {
        TreeGridHead treeGridHead = new TreeGridHead(5, this.cols, this.colKeys, this.excludeCols, this.columnFormat);
        (treeGridHead.getColumns().get(0)).setWidth("300");
        switch(typeEnum) {
            case HEAD:
            ...
            case BODY:
                ...
                List<Map<String, String>> dataList = this.treeGridInfra.queryKylinDataByTemplateForTree(params, treeGridHead.getId(), treeGridHead.getParentId(), this.colKeys, "GMV/GMV品类明细.ftl", level, treeValue);
                return dataList;
            default:
